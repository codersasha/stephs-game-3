...
(function () {
  ...
  /* ====================================================
     DOG PACK — 3D models for the dog attack event
     ==================================================== */
  let dogModels = []; // active dog 3D groups in the scene

  function makeDog (color, size, px, pz) {
    const g = new THREE.Group();
    const sz = size || 1.6;
    const furMat = new THREE.MeshPhongMaterial({ color: color || 0x664422, shininess: 3 });
    const bellyMat = new THREE.MeshPhongMaterial({ color: 0xaa8866, shininess: 5 });

    // Body — larger and more muscular than a cat
    var bodyR = 0.42 * sz, bodyL = 0.95 * sz;
    var body = makeCapsuleMesh(bodyR, bodyL, 12, 16, furMat);
    body.rotation.x = Math.PI / 2; body.position.y = 0.75 * sz; body.castShadow = true;
    g.add(body);
    // Belly
    var bellyM = makeCapsuleMesh(bodyR * 0.72, bodyL * 0.60, 8, 12, bellyMat);
    bellyM.rotation.x = Math.PI / 2; bellyM.position.set(0, 0.62 * sz, 0.03);
    g.add(bellyM);

    // Head — bigger, blockier snout
    var headR = 0.32 * sz;
    var head = new THREE.Mesh(new THREE.SphereGeometry(headR, 12, 10), furMat);
    head.position.set(0, 0.95 * sz, 0.65 * sz); head.scale.set(1.1, 0.95, 1.1);
    head.castShadow = true; g.add(head);

    // Snout / muzzle — longer than a cat's
    var snoutMat = new THREE.MeshPhongMaterial({ color: 0x886655, shininess: 3 });
    var snout = new THREE.Mesh(new THREE.SphereGeometry(0.18 * sz, 10, 8), snoutMat);
    snout.position.set(0, 0.88 * sz, 0.98 * sz); snout.scale.set(0.9, 0.65, 1.3);
    g.add(snout);

    // Nose
    var nose = new THREE.Mesh(new THREE.SphereGeometry(0.06 * sz, 8, 6), new THREE.MeshPhongMaterial({ color: 0x222222 }));
    nose.position.set(0, 0.90 * sz, 1.12 * sz); g.add(nose);

    // Eyes — angry, small
    [[-1,1],[1,1]].forEach(function (pair) {
      var s = pair[0];
      var eye = new THREE.Mesh(new THREE.SphereGeometry(0.065 * sz, 10, 8), new THREE.MeshPhongMaterial({ color: 0xddaa33, shininess: 12 }));
      eye.position.set(s * 0.14 * sz, 1.0 * sz, 0.88 * sz); eye.scale.set(1, 0.7, 0.5);
      g.add(eye);
      var pupil = new THREE.Mesh(new THREE.SphereGeometry(0.035 * sz, 8, 6), new THREE.MeshBasicMaterial({ color: 0x111111 }));
      pupil.position.set(s * 0.14 * sz, 1.0 * sz, 0.91 * sz); pupil.scale.set(0.6, 0.6, 0.4);
      g.add(pupil);
    });

    // Ears — floppy/pointed depending on dog
    [[-1,1],[1,1]].forEach(function (pair) {
      var s = pair[0];
      var ear = new THREE.Mesh(new THREE.ConeGeometry(0.12 * sz, 0.22 * sz, 4), furMat);
      ear.position.set(s * 0.18 * sz, 1.18 * sz, 0.58 * sz);
      ear.rotation.z = s * 0.4; ear.rotation.x = 0.3;
      ear.castShadow = true; g.add(ear);
    });

    // Mouth — open, showing teeth
    var jawMat = new THREE.MeshPhongMaterial({ color: 0x882222 });
    var jaw = new THREE.Mesh(new THREE.SphereGeometry(0.1 * sz, 8, 6), jawMat);
    jaw.position.set(0, 0.82 * sz, 1.02 * sz); jaw.scale.set(0.8, 0.3, 0.7);
    g.add(jaw);
    // Teeth
    var toothMat = new THREE.MeshPhongMaterial({ color: 0xffffff, shininess: 20 });
    [[-0.04, 0.04], [0.04, 0.04]].forEach(function (off) {
      var tooth = new THREE.Mesh(new THREE.ConeGeometry(0.02 * sz, 0.06 * sz, 4), toothMat);
      tooth.position.set(off[0] * sz, 0.82 * sz, 1.08 * sz); tooth.rotation.x = Math.PI;
      g.add(tooth);
    });

    // 4 legs — thicker than cat legs
    var legMat = new THREE.MeshPhongMaterial({ color: color || 0x664422, shininess: 3 });
    var legPositions = [
      { x: -0.18 * sz, z: 0.35 * sz },
      { x: 0.18 * sz, z: 0.35 * sz },
      { x: -0.18 * sz, z: -0.35 * sz },
    ...
    legPositions.forEach(function (lp) {
      var leg = new THREE.Mesh(new THREE.CylinderGeometry(0.08 * sz, 0.07 * sz, 0.55 * sz, 8), legMat);
      leg.position.set(lp.x, 0.28 * sz, lp.z); leg.castShadow = true;
      g.add(leg); g.legs.push(leg);
      // Paw
      var paw = new THREE.Mesh(new THREE.SphereGeometry(0.09 * sz, 8, 6), legMat);
      paw.position.set(lp.x, 0.04 * sz, lp.z); paw.scale.set(1, 0.5, 1.2);
      g.add(paw);
    });

    // Tail — thick, slightly curved up
    var tailMat = new THREE.MeshPhongMaterial({ color: color || 0x664422, shininess: 3 });
    for (var ti = 0; ti < 4; ti++) {
      var seg = new THREE.Mesh(new THREE.SphereGeometry((0.07 - ti * 0.01) * sz, 8, 6), tailMat);
      seg.position.set(0, (0.80 + ti * 0.06) * sz, (-0.55 - ti * 0.12) * sz);
      g.add(seg);
    }

    g.position.set(px || 0, 0, pz || 0);
    scene.add(g);
    return g;
  }

  function spawnDogPack (positions) {
    removeDogPack(); // clear any existing dogs
    var dogColors = [0x664422, 0x554433, 0x887755, 0x443322, 0x776644, 0x332211];
    var dogSizes = [1.6, 1.8, 1.5, 1.7, 1.9, 1.6];
    positions.forEach(function (pos, i) {
      var dog = makeDog(dogColors[i % dogColors.length], dogSizes[i % dogSizes.length], pos.x, pos.z);
      dogModels.push(dog);
    });
  }

  function removeDogPack () {
    dogModels.forEach(function (d) { scene.remove(d); });
    dogModels = [];
  }

  function animateDogPack (dt) {
    // Simple menacing animation — dogs shift and snarl
    dogModels.forEach(function (dog, i) {
      if (!dog.legs) return;
      var t = (performance.now() / 1000) + i * 1.5;
      // Leg movement — pacing/shifting
      var swing = Math.sin(t * 4) * 0.2;
      dog.legs[0].rotation.x = swing;
      dog.legs[1].rotation.x = -swing;
      dog.legs[2].rotation.x = -swing;
      dog.legs[3].rotation.x = swing;
      // Slight body bob
      dog.children[0].position.y = 0.75 * 1.6 + Math.sin(t * 3) * 0.02;
    });
  }
  ...
  function triggerDangerousPath () {
    ...
    // Spawn the dog pack at the camp entrance so they are VISIBLE
    spawnDogPack([
      { x: 8, z: 18 }, { x: 12, z: 20 }, { x: 5, z: 22 },
      { x: 15, z: 17 }, { x: 10, z: 24 }, { x: 3, z: 19 }
    ]);
    ...
    const scenes = [
      { narration: true, text: 'Dark times fall upon the forest. A pack of savage dogs has been sighted in ThunderClan territory — huge, vicious beasts that kill without mercy.',
        camPos: { x: 0, y: 6, z: 10 }, camLook: { x: 10, y: 1, z: 20 } },
      { narration: true, text: 'You hear them before you see them — deep, guttural growling coming from the undergrowth. Then you spot them: SIX enormous dogs, their teeth bared and eyes wild!',
        camPos: { x: 8, y: 3, z: 15 }, camLook: { x: 10, y: 1, z: 20 } },
      ...
      { narration: true, text: '<em>Tigerclaw.</em> Even in exile, he plots to destroy ThunderClan. He is leading the dog pack right to your camp!',
        camPos: { x: 12, y: 4, z: 22 }, camLook: { x: 10, y: 1, z: 20 } },
    ...
    startCutscene(scenes, () => {
      gameState = 'playing'; placeCatsInCamp(); saveGame();
      queueMessage('Narrator', 'Tigerclaw is luring a dog pack to ThunderClan\'s camp! You can see the dogs prowling near the entrance! Organize a plan to lead them toward the gorge!');
      showNextChapterButton();
    });
  }
  ...
  function triggerDogPack () {
    ...
    // Move dogs along the chase path during cutscene
    // If dogs aren't spawned yet, spawn them at forest edge
    if (dogModels.length === 0) {
      spawnDogPack([
        { x: 8, z: 18 }, { x: 12, z: 20 }, { x: 5, z: 22 },
        { x: 15, z: 17 }, { x: 10, z: 24 }, { x: 3, z: 19 }
      ]);
    ...
    const scenes = [
      { narration: true, text: 'The plan is set! A relay of ThunderClan\'s fastest cats will lure the dog pack along a trail, one by one, leading them toward the river gorge!',
        camPos: { x: 0, y: 6, z: 10 }, camLook: { x: 10, y: 1, z: 20 } },
      { narration: true, text: 'The howling of dogs echoes through the forest! The pack charges toward camp — six massive beasts with drool flying from their jaws!',
        camPos: { x: 10, y: 4, z: 18 }, camLook: { x: 12, y: 1, z: 20 },
        onShow: function () {
          // Move dogs toward the forest trail
          dogModels.forEach(function (d, i) {
            d.position.set(15 + i * 3, 0, 20 + i * 2);
            d.rotation.y = -Math.PI * 0.3;
          });
        } },
      { narration: true, text: 'One by one, each cat runs as fast as they can before the next takes over. The dogs follow, snarling and snapping at every tail!',
        ...
        onShow: function () {
          dogModels.forEach(function (d, i) {
            d.position.set(35 + i * 3, 0, 8 + i * 2);
            d.rotation.y = -Math.PI * 0.1;
          });
        } },
      { narration: true, text: 'You are the last cat in the relay — the anchor! The dogs barrel toward you, their eyes wild with bloodlust!',
        camPos: { x: 60, y: 3, z: -5 }, camLook: { x: 55, y: 1, z: -3 },
        onShow: function () {
          dogModels.forEach(function (d, i) {
            d.position.set(55 + i * 2, 0, -3 + i);
            d.rotation.y = Math.PI * 0.5;
          });
        } },
    ...
    startCutscene(scenes, () => {
      removeDogPack(); // make sure dogs are cleaned up
      gameState = 'playing'; placeCatsInCamp(); saveGame();
  ...
  function triggerBluestarLastLife () {
    ...
    const scenes = [
      ...
      { speaker: 'Bluestar', text: '"Tell ThunderClan... I am proud of every one of them. You will lead them well... ' + pName.replace(new RegExp('(' + WARRIOR_SUFFIXES.join('|') + '|paw)$', 'i'), '') + 'star."',
  ...
  function triggerTigerClanRises () {
    ...
    playSound('danger');
    const scenes = [
      { narration: true, text: 'A moon has passed since your leadership ceremony. The forest is changing — and dark rumors reach ThunderClan...',
        camPos: { x: 0, y: 8, z: 10 }, camLook: { x: 0, y: 2, z: 0 },
        onShow: function () { setDayMode(); } },
      { speaker: 'Whitestorm', text: '"' + pName + ', I have grave news. Scouts report that Tigerclaw — the exile — has taken control of ShadowClan."',
        camPos: { x: -1, y: 2, z: 1 }, camLook: { x: 2, y: 1, z: 0 } },
      { speaker: pName, text: '"What?! How is that possible? ShadowClan would never accept an outsider!"',
        camPos: { x: 2, y: 2, z: 0 }, camLook: { x: -1, y: 1, z: 1 } },
      { speaker: 'Whitestorm', text: '"He killed their leader and claimed the name <strong>Tigerstar</strong>. He received nine lives from the Dark Forest itself. He is more dangerous than ever."',
        camPos: { x: -1, y: 2, z: 1 }, camLook: { x: 2, y: 1, z: 0 } },
      { narration: true, text: 'That night, Tigerstar calls a Gathering at Fourtrees. All four Clans are summoned.',
        camPos: { x: -43, y: 8, z: -43 }, camLook: { x: -45, y: 1.5, z: -45 },
        onShow: function () { setNightMode(); } },
      { narration: true, text: 'The Great Rock gleams under the full moon. Cats from every Clan crowd the hollow. The air is thick with tension.',
        camPos: { x: -44, y: 4, z: -44 }, camLook: { x: -45, y: 2, z: -45 } },
      { speaker: 'Tigerstar', text: '"Cats of all Clans! The time of four Clans is OVER. I have united ShadowClan and RiverClan under one banner — <strong>TigerClan</strong>!"',
        camPos: { x: -45, y: 3, z: -44 }, camLook: { x: -45, y: 2.5, z: -45 } },
      { narration: true, text: 'Shocked murmurs ripple through the crowd. RiverClan cats hang their heads — they had no choice. Tigerstar\'s warriors stand behind him, claws unsheathed.',
        camPos: { x: -43, y: 3, z: -46 }, camLook: { x: -45, y: 1, z: -45 } },
      { speaker: 'Tigerstar', text: '"ThunderClan. WindClan. Join TigerClan now, and you will be spared. Resist, and you will be destroyed. There is no third option."',
        camPos: { x: -45, y: 3, z: -44 }, camLook: { x: -45, y: 2.5, z: -45 } },
      { speaker: pName, text: '"NEVER! The forest needs four Clans, Tigerstar. Your hunger for power will destroy everything the warrior code stands for!"',
        camPos: { x: -44, y: 2.5, z: -44 }, camLook: { x: -45, y: 2, z: -45 } },
      { speaker: 'Tigerstar', text: '"Then you have chosen DEATH, ' + pName + '. I will bring a force so powerful that not even StarClan can save you. You have been warned."',
        camPos: { x: -45, y: 3, z: -43 }, camLook: { x: -44, y: 2, z: -44 } },
      { narration: true, text: 'Tigerstar leaps down from the Great Rock and stalks away into the darkness, his warriors following like shadows.',
        camPos: { x: -46, y: 4, z: -42 }, camLook: { x: -48, y: 1, z: -45 } },
      { speaker: 'Whitestorm', text: '"He spoke of bringing cats from beyond the forest... Twolegplace rogues. A group called <strong>BloodClan</strong>."',
        camPos: { x: -43, y: 2, z: -43 }, camLook: { x: -44, y: 1, z: -44 } },
      { narration: true, text: '<em>War is coming. Tigerstar has formed TigerClan and threatens to destroy everything. He speaks of BloodClan — a terrifying army of rogues from Twolegplace...</em>',
        camPos: { x: 0, y: 10, z: 5 }, camLook: { x: 0, y: 2, z: 0 },
        onShow: function () { setDayMode(); } },
    ];
    startCutscene(scenes, () => {
      gameState = 'playing'; setDayMode(); placeCatsInCamp(); saveGame();
  ...
  function triggerBloodClanArrives () {
    ...
    playSound('danger');
    const scenes = [
      { narration: true, text: 'Tigerstar has called another Gathering at Fourtrees. This time, he says, the forest cats will see the full power of TigerClan.',
        camPos: { x: -43, y: 8, z: -43 }, camLook: { x: -45, y: 1.5, z: -45 },
        onShow: function () { setNightMode(); } },
      { narration: true, text: 'The moon hangs full and cold above Fourtrees. All four Clans are gathered — ThunderClan, WindClan, ShadowClan, and RiverClan. The hollow is packed with cats.',
        camPos: { x: -44, y: 5, z: -44 }, camLook: { x: -45, y: 1.5, z: -45 } },
      { speaker: 'Tigerstar', text: '"Cats of the forest! Tonight I present to you the army that will bring the forest under ONE rule — MY rule!"',
        camPos: { x: -45, y: 3, z: -44 }, camLook: { x: -45, y: 2.5, z: -45 } },
      { narration: true, text: 'From the shadows beyond the hollow, dozens of cats emerge. They are scarred, lean, and vicious. Their eyes gleam with cruelty. Dog teeth and cat claws are woven into their collars.',
        camPos: { x: -48, y: 3, z: -42 }, camLook: { x: -50, y: 1, z: -44 } },
      { narration: true, text: 'This is <strong>BloodClan</strong>.',
        camPos: { x: -49, y: 2, z: -43 }, camLook: { x: -50, y: 0.8, z: -44 } },
      { narration: true, text: 'At their head stands a small, jet-black tom with one white paw. His collar bristles with the teeth and claws of cats he has killed. His ice-blue eyes are colder than leaf-bare frost.',
        camPos: { x: -47, y: 1.8, z: -44.5 }, camLook: { x: -48, y: 0.8, z: -45 } },
      { narration: true, text: 'This is <strong>Scourge</strong>, leader of BloodClan.',
        camPos: { x: -47.5, y: 1.5, z: -44.5 }, camLook: { x: -48, y: 0.6, z: -45 } },
      { speaker: 'Tigerstar', text: '"Scourge! I brought you here as promised. Now — DESTROY ThunderClan! Tear them apart!"',
        camPos: { x: -45, y: 3, z: -44 }, camLook: { x: -45, y: 2.5, z: -45 } },
      { narration: true, text: 'Scourge does not move. He stares up at Tigerstar with those frozen blue eyes. The silence stretches on. Every cat holds their breath.',
        camPos: { x: -47, y: 2, z: -44.5 }, camLook: { x: -46, y: 1.5, z: -45 } },
      { speaker: 'Scourge', text: '"I don\'t take orders from anyone. Least of all from you."',
        camPos: { x: -48, y: 1.5, z: -44.5 }, camLook: { x: -47.5, y: 0.8, z: -45 } },
      { speaker: 'Tigerstar', text: '"WHAT?! We had a deal, Scourge! I brought you to the forest — you answer to ME!"',
        camPos: { x: -45, y: 3, z: -44 }, camLook: { x: -47, y: 1, z: -45 } },
      { narration: true, text: 'Tigerstar lunges at Scourge with a furious yowl — but Scourge is faster. <strong>Much</strong> faster.',
        camPos: { x: -46, y: 2.5, z: -44.5 }, camLook: { x: -47, y: 1, z: -45 } },
      { narration: true, text: '<strong>SLASH!</strong> Scourge\'s reinforced claws — studded with dogs\' teeth — rip across Tigerstar\'s throat in a single, devastating blow!',
        camPos: { x: -46.5, y: 1.5, z: -44.5 }, camLook: { x: -46, y: 0.3, z: -45 } },
      { narration: true, text: 'Tigerstar crashes to the ground. His body convulses as he loses a life... then another... then another. His eyes widen in horror as life after life is torn away.',
        camPos: { x: -46, y: 1, z: -44.5 }, camLook: { x: -46, y: 0.2, z: -45 } },
      { speaker: 'Tallstar', text: '"StarClan, no... He\'s losing ALL of his lives at once!"',
        camPos: { x: -43, y: 2, z: -43 }, camLook: { x: -46, y: 0.3, z: -45 } },
      { narration: true, text: 'Every cat watches in frozen horror. Tigerstar writhes, gasps, and goes still. Then writhes again. Nine times his body shudders... and nine times he dies.',
        camPos: { x: -45, y: 2, z: -44 }, camLook: { x: -46, y: 0.1, z: -45 } },
      { narration: true, text: '<strong>Tigerstar is dead.</strong> All nine lives, ripped away with a single blow. The most powerful cat in the forest — killed in an instant.',
        camPos: { x: -46, y: 1.5, z: -44.5 }, camLook: { x: -46, y: 0, z: -45 } },
      { narration: true, text: 'The forest has never seen anything like it. Cats from every Clan stare in disbelief. Even Tigerstar\'s own warriors back away in terror.',
        camPos: { x: -44, y: 4, z: -43 }, camLook: { x: -46, y: 0.5, z: -45 } },
      { narration: true, text: 'Scourge calmly licks the blood from his claws. Then he turns his ice-blue gaze on the gathered Clans.',
        camPos: { x: -47, y: 1.8, z: -44.5 }, camLook: { x: -47.5, y: 0.8, z: -45 } },
      { speaker: 'Scourge', text: '"I am the leader now. This forest belongs to BloodClan."',
        camPos: { x: -47.5, y: 1.5, z: -44.5 }, camLook: { x: -47.5, y: 0.8, z: -45 } },
      { speaker: 'Scourge', text: '"You have three days to leave. Every cat who remains after that... will share Tigerstar\'s fate."',
        camPos: { x: -48, y: 1.5, z: -44 }, camLook: { x: -47, y: 0.8, z: -45 } },
      { narration: true, text: 'Scourge turns and walks away into the darkness. BloodClan follows, their eyes gleaming. The Clans are left alone with Tigerstar\'s body.',
        camPos: { x: -44, y: 5, z: -43 }, camLook: { x: -49, y: 1, z: -46 } },
      { speaker: pName, text: '"We can\'t let this happen. We won\'t run. The forest is OUR home — ALL four Clans\' home."',
        camPos: { x: -44, y: 2.5, z: -44 }, camLook: { x: -45, y: 2, z: -45 } },
      { speaker: pName, text: '"I know we\'ve had our differences. But if we don\'t stand together now, BloodClan will destroy us all. Will you fight with me?"',
        camPos: { x: -44, y: 2.5, z: -44 }, camLook: { x: -43, y: 1.5, z: -43 } },
      { speaker: 'Tallstar', text: '"WindClan stands with you, ' + pName + '. We will not be driven from our home."',
        camPos: { x: -43, y: 2, z: -46 }, camLook: { x: -44, y: 1.5, z: -45 } },
      { speaker: 'Leopardstar', text: '"...RiverClan will fight. Tigerstar forced us into TigerClan. We owe you this."',
        camPos: { x: -46, y: 2, z: -43 }, camLook: { x: -45, y: 1.5, z: -44 } },
      { narration: true, text: 'Even the remaining ShadowClan warriors nod their heads. For the first time in the history of the forest, ALL FOUR CLANS will fight as one.',
        camPos: { x: -45, y: 4, z: -45 }, camLook: { x: -45, y: 1, z: -45 } },
      { speaker: pName, text: '"Then we are <strong>LionClan</strong> — united under one banner. And we will drive BloodClan from this forest TOGETHER!"',
        camPos: { x: -44, y: 3, z: -44 }, camLook: { x: -45, y: 2, z: -45 } },
      { narration: true, text: '<em>Tigerstar is dead — killed by the very monster he unleashed. Now BloodClan threatens to destroy everything. The forest\'s only hope is for all four Clans to unite as LionClan — under your leadership.</em>',
        camPos: { x: 0, y: 10, z: 5 }, camLook: { x: 0, y: 2, z: 0 },
        onShow: function () { setDayMode(); } },
    ];
    startCutscene(scenes, () => {
      gameState = 'playing'; setDayMode(); placeCatsInCamp(); saveGame();
      queueMessage('Narrator', 'Scourge has killed Tigerstar and threatens to take the forest. You must unite ALL four Clans — ThunderClan, WindClan, RiverClan, and ShadowClan — to fight BloodClan together as LionClan. This is the final battle.');
      showNextChapterButton();
    });
  }
  ...
  function triggerScourge () {
    ...
    playSound('danger');
    const scenes = [
      { narration: true, text: 'The third day has come. Dawn breaks over Fourtrees — and LionClan is ready.',
        camPos: { x: -43, y: 8, z: -43 }, camLook: { x: -45, y: 1, z: -45 },
        onShow: function () { setDayMode(); } },
      { narration: true, text: 'All four Clans stand shoulder to shoulder — ThunderClan, WindClan, RiverClan, and ShadowClan. Warriors from every Clan, united as one. This has never happened before.',
        camPos: { x: -44, y: 4, z: -44 }, camLook: { x: -45, y: 1.5, z: -45 } },
      { speaker: pName, text: '"Remember — we fight as ONE Clan today. LionClan! Protect each other! No cat fights alone!"',
        camPos: { x: -44, y: 3, z: -44 }, camLook: { x: -45, y: 2, z: -45 } },
      { speaker: 'Whitestorm', text: '"Whatever happens today, ' + pName + ', it has been an honor to serve ThunderClan under your leadership."',
        camPos: { x: -43, y: 2, z: -43 }, camLook: { x: -44, y: 1.5, z: -44 } },
      { narration: true, text: 'Then you hear them. Paw steps. Hundreds of paw steps, thundering through the undergrowth like a storm.',
        camPos: { x: -48, y: 3, z: -42 }, camLook: { x: -50, y: 1, z: -44 } },
      { narration: true, text: 'BloodClan pours out of the shadows. Scarred, battle-hardened rogues with reinforced claws and collars of teeth. Their eyes show no mercy, no fear, no honor.',
        camPos: { x: -50, y: 4, z: -40 }, camLook: { x: -48, y: 1, z: -42 } },
      { narration: true, text: 'Scourge stands at their head. Small, black, and utterly terrifying. His ice-blue eyes sweep across LionClan with cold contempt.',
        camPos: { x: -47, y: 1.8, z: -44.5 }, camLook: { x: -48, y: 0.8, z: -45 } },
      { speaker: 'Scourge', text: '"Your three days are up. I gave you a chance to leave. Now BloodClan will take this forest — starting with your lives."',
        camPos: { x: -48, y: 1.5, z: -44.5 }, camLook: { x: -47.5, y: 0.8, z: -45 } },
      { speaker: pName, text: '"This is OUR forest, Scourge! We were born here, we live here, and we will FIGHT for it! LionClan — ATTACK!"',
        camPos: { x: -44, y: 3, z: -44 }, camLook: { x: -47, y: 1, z: -45 } },
      { narration: true, text: '<strong>The biggest battle the forest has ever known begins!</strong> Cats clash on every side! Yowls and shrieks fill the air! BloodClan fights dirty, but LionClan fights with the courage of StarClan!',
        camPos: { x: -44, y: 6, z: -44 }, camLook: { x: -46, y: 1, z: -46 } },
      { narration: true, text: 'You fight your way through the chaos — clawing, dodging, leaping — until you stand face to face with Scourge himself.',
        camPos: { x: -46, y: 2, z: -45.5 }, camLook: { x: -47, y: 0.8, z: -45 } },
      { speaker: 'Scourge', text: '"I killed Tigerstar with one blow. Nine lives, gone — just like that. You will be no different, forest fool."',
        camPos: { x: -47, y: 1.5, z: -45 }, camLook: { x: -47, y: 0.8, z: -45 } },
      { speaker: pName, text: '"Tigerstar was a murderer and a coward. I am NOTHING like him. And unlike him — I have something worth fighting for."',
        camPos: { x: -46, y: 2, z: -45.5 }, camLook: { x: -47, y: 0.8, z: -45 } },
      { narration: true, text: 'Scourge strikes first! His dog-tooth claws tear through your flesh — agony rips through your body! The world goes dark...',
        camPos: { x: -46, y: 1.5, z: -45 }, camLook: { x: -46.5, y: 0.5, z: -45 } },
      { narration: true, text: 'You lose a life. Your body goes limp. BloodClan cats yowl in triumph — they think it\'s over.',
        camPos: { x: -46, y: 1, z: -45 }, camLook: { x: -46.5, y: 0.2, z: -45 } },
      { narration: true, text: 'But then... warmth floods through you. StarClan\'s light surrounds you. Your eyes snap open. You leap to your paws! <strong>You have nine lives. Scourge only has one.</strong>',
        camPos: { x: -46, y: 2, z: -45 }, camLook: { x: -46, y: 1, z: -45 } },
      { speaker: 'Scourge', text: '"IMPOSSIBLE! You... you should be DEAD!"',
        camPos: { x: -47, y: 1.5, z: -45 }, camLook: { x: -47, y: 0.8, z: -45 } },
      { speaker: pName, text: '"StarClan gave me nine lives, Scourge. And I only need ONE of them to defeat you!"',
        camPos: { x: -46, y: 2, z: -45 }, camLook: { x: -47, y: 1, z: -45 } },
    ];
    startCutscene(scenes, () => {
  ...
  function startPlaying () {
    ...
    catGroup.visible = true;
    removeDogPack(); // clear any leftover dog models
  ...
  function animate () {
    ...
    // Always animate tails, legs, fireflies, water, dogs regardless of state
    animateFireflies(time);
    animateTail(time);
    animateNPCTails(time);
    animateNPCLegs(dt);
    animateWater(time);
    if (dogModels.length > 0) animateDogPack(dt);
